...
ADHD 치료제 품귀의 핵심 원인 중 하나는 미국 DEA(마약단속청)의 시대착오적인 할당량 시스템이다. 1970년 제정된 통제물질법에 따라, DEA는 매년 자극제의 총 생산량을 미리 정해놓는다. 문제는 이 할당량이 실제 의료 수요와 전혀 맞지 않는다는 것이다.

**할당량 시스템의 작동 방식:**
1. DEA가 전년도 데이터를 바탕으로 다음 해 할당량 결정
2. 제약회사들이 할당량 범위 내에서만 생산 가능
3. 할당량 초과 시 형사처벌 위험

2023년 11월 DEA는 충격적인 사실을 발표했다. 암페타민 계열 약물 제조업체들이 2022년 한 해 동안 필요량보다 10억 용량을 적게 생산했다는 것이다. 이는 할당량 부족 때문이었다.

**구체적 사례: 테바 제약의 딜레마**

이스라엘의 제네릭 의약품 회사 테바는 미국에서 가장 큰 제네릭 애더럴 생산업체다. 2022년 테바의 내부 문서에 따르면, 회사는 시장 수요를 충족할 수 있는 생산 능력을 갖추고 있었지만 DEA 할당량 때문에 생산을 30% 줄여야 했다.

테바의 한 임원은 익명을 조건으로 "우리는 환자들이 필요로 하는 약을 만들 수 있지만, 법적으로 만들 수 없다. 이는 환자보다 규제를 우선시하는 시스템"이라고 비판했다.
...
가장 시급한 것은 DEA 할당량 시스템의 현실화다. 현재 시스템은 50년 전 불법 약물 남용 방지를 목적으로 설계됐지만, 현재는 오히려 합법적 의료 수요를 막는 장벽이 되고 있다.

미국 의료진들은 다음과 같은 개선 방안을 제시하고 있다:
- 의료 수요 기반 할당량 산정 방식 도입
- 긴급상황 시 할당량 추가 배정 시스템 구축
- 분기별 할당량 조정 시스템 도입
